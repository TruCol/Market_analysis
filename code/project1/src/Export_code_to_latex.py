...
def export_code_to_latex(main_latex_filename, project_nr):
    """This function exports the python files and compiled pdfs of jupiter
...
def create_appendices_latex_code(
    ...
    """Creates the latex code that includeds the appendices in the main latex
    file.
    ...
    main_appendix_inclusion_lines.append(
        rf"\IfFileExists{{latex/project{project_nr}/main.tex}}{{"
    )
...
def filter_appendices_by_type(appendices, appendix_type):
    """Returns the list of all appendices of a certain appendix type, from the
    incoming list of Appendix objects.
...
def sort_appendices_on_code_filename(appendices):
    """Returns a list of Appendix objects that are sorted and  based on the
    property: code_filename. Assumes the incoming appendices only contain
    python files.
    ...
    sorted_indices = sorted(
        range(len(attributes)), key=lambda k: attributes[k]
    )
    sorted_list = []
...
def get_order_of_non_code_appendices_in_main(appendices, appendix_tex_code):
    """Scans the lines of appendices in the main code, and returns the lines of
    the appendices that do not contain code, in the order in which they were
...
def get_filename_from_latex_appendix_line(appendices, appendix_line):
    """Returns the first filename from a list of incoming filenames that occurs
    in a latex code line.
    ...
    for filename in list(
        map(lambda appendix: appendix.appendix_filename, appendices)
    ):
...
def get_appendix_from_filename(appendices, appendix_filename):
    """Returns the first Appendix object with an appendix filename that matches
    the incoming appendix_filename. The Appendix objects are selected from an
    incoming list of Appendix objects.
...
def get_compiled_notebook_paths(script_dir):
    """Returns the list of jupiter notebook filepaths that were compiled
    successfully and that are included in the same dias this script (the src
    directory).
...
def get_list_of_appendix_files(
    ...
    """Returns a list of Appendix objects that contain all the appendix files
    with .tex extension.
    ...
    for appendix_filepath in appendices_paths:
        ...
        line_nr_python_file_inclusion = get_line_of_latex_command(
            appendix_filecontent, r"\pythonexternal{"
        )
        line_nr_notebook_file_inclusion = get_line_of_latex_command(
            appendix_filecontent, r"\includepdf[pages="
        )
        ...
        if line_nr_notebook_file_inclusion > -1:
            ...
            appendices.append(
                Appendix(
                    appendix_filepath, appendix_filecontent, appendix_type
                )
            )
    return appendices
...
def get_filename_from_latex_inclusion_command(
    ...
    """returns the code/notebook filename in a latex command which includes
    that code in an appendix. The inclusion command includes a python code or
    jupiter notebook pdf.
...
def get_code_files_already_included_in_appendices(
    ...
    """Returns a list of code filepaths that are already properly included the
    latex appendix files of this project.
...
def check_if_appendix_contains_file(
    ...
    """Scans an appendix content to determine whether it contains a substring
    that includes a code file (of either python or compiled notebook=pdf
    extension).
...
def get_line_of_latex_command(appendix_content, latex_command):
    """Returns the line number of a latex command if it is found. Returns -1
    otherwise.
...
def get_latex_inclusion_command(
    extension, latex_relative_filepath_to_codefile
):
    """Creates and returns a latex command that includes either a python file
    or a compiled jupiter notebook pdf (whereever the command is placed). The
...
def read_file(filepath):
    """Reads content of a file and returns it as a list of strings, with one
    string per line.
...
def get_code_files_not_yet_included_in_appendices(
    ...
    """Returns a list of filepaths that are not yet properly included in some
    ...
    contained_filepaths = list(
        map(
            lambda contained_file: contained_file.code_filepath,
            contained_codes,
        )
    )
    not_contained = []
...
def add_include_code_in_appendix(
    ...
    """
    content.append(
        rf"\IfFileExists{{latex/project{project_nr}/../../{code_filepath[len(root_dir):]}}}{{"
    )
    # append current line
    content.append(
        get_latex_inclusion_command(extension, latex_relative_filepath)
    )
    content.append(f"}}{{")
    content.append(
...
def get_index_of_auto_generated_appendices(appendix_dir, extension):
    """Returns the maximum index of auto generated appendices of a specific
    extension type.
...
def get_auto_generated_appendix_filenames_of_specific_extension(
    ...
    """Returns the list of auto generated appendices of a specific extension
    type.
...
def create_section(appendix_reference_index, code_filename, content):
    """Creates the header of a latex appendix file, such that it contains a
    section that indicates the section is an appendix, and indicates which
    pyhon or notebook file is being included in that appendix.
    ...
    """
    # write section
    left = r"\section{Appendix "
    middle = code_filename.replace("_", r"\_")
    right = r"}\label{app:"
...
def overwrite_content_to_file(content, filepath, content_has_newlines=True):
    """Writes a list of lines of tex code from the content argument to a .tex
    file using overwriting method. The content has one line per element.
...
def get_index_of_substring_in_list(lines, target_substring):
    """Returns the index of the line in which the first character of a latex
    substring if it is found uncommented in the incoming list.
...
def update_appendix_tex_code(appendix_filename, is_from_root_dir, project_nr):
    """Returns the latex command that includes an appendix .tex file in an
    appendix environment as can be used in the main tex file.

    :param appendix_filename: Name of the appendix that is included by the generated command.
    :param project_nr: The number indicating which project this code pertains to.
    """
    if is_from_root_dir:
        left = rf"\input{{latex/project{project_nr}/"
    else:
        left = r"\input{"
    middle = "Appendices/"
    right = "} \\newpage\n"
...
def substitute_appendix_code(
    ...
    """Replaces the old latex code that included the appendices in the main.tex
    file with the new latex commands that include the appendices in the latex
    report.
...
class Appendix_with_code:
    """stores in which appendix file and accompanying line number in the
    appendix in which a code file is already included.

    Does not take into account whether this appendix is in the main tex
    file or not
    """
...
class Appendix:
    ...
    def __init__(
        ...
        self.appendix_content = appendix_content
        self.appendix_type = (
            appendix_type  # TODO: perform validation of input values
        )
